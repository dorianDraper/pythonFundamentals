...
Now, based on this data set, Python can create a decision tree that can be used
to decide if any new shows are worth attending to.
...
The decision tree uses your earlier decisions to calculate the odds for you
...
Rank
    ...
    between 0.0 and 0.5, where 0.0 would mean all of the samples got the same result,
...
GINI coefficient is a method used in decision tree algorithms to decide the
...
The next step contains two boxes:
    one box for the comedians with a 'Rank' of 6.5 or lower,
...
Nationality
    ...
    Nationality <= 0.5 means that the comedians with a nationality value of
    less than 0.5 will follow the arrow to the left (which means everyone from the UK, ),
