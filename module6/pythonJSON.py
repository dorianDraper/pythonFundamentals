...
Example: Import the json module: import json
"""
"""
...
x = '{"name": "Jorge", "age": 34, "city": "Esplugues"}'

y = json.loads(x)

print (y)	#the result is a Python dictionary
print (y["age"])	#one value of our Python dictionary
...
by using the json.dumps() method. Example: Convert from Python to JSON:
"""
x = {
	"name": "Lupin",
	"platform": "Netflix",
...
#we can also create an object y = json.dumps(x) and print (y)
print (type(x))
...
a comma and a space to separate each object, and a colon and a space to separate keys from values:
